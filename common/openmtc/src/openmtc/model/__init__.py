...
class Entity(LoggerMixin):
    ...
    def get_attribute_values(self, filter=False):
        return {
            attr.name: getattr(self, attr.name) for attr in self.attributes
            if not (filter and getattr(self, attr.name) in (None, '', []))
        }
    ...
    def subresource_values(self):
        return {
            resource.name: getattr(self, resource.name) for resource in self.subresources
        }
...
class Resource(Entity):
    ...
    def set_values(self, values):
        ...
        for key in [key for key in values.keys() if '_' in key]:
