...
class Promise(LoggerMixin):
    ...
    def fulfill(self, value):
        ...
        #print ("fulfill", value, self._callbacks)
        ...
        for callback in self._callbacks:
            try:
                #print "call", callback
                #self.logger.debug("Calling callback %s", callback)
                callback(value)
            except Exception:
    ...
    def reject(self, reason, bubbling=False):
        """
        ...
        if not bubbling and log_error(reason):
            ...
            self.logger.debug("Promise (%s) rejected: %s", self.name, reason, exc_info = exc_info[0] and exc_info or None)
            self.logger.debug(self._errbacks)
            print_stack()
        else:
            pass
            #self.logger.debug("Promise (%s) rejected", self.name)
            #self.logger.debug(self._errbacks)
    ...
    def _invoke(self, func, value):
        try:
            #self.logger.debug("Invoking %s with %s", func, value)
            if value is None:
                """
                if ismethod(func):
                    self.logger.debug("its a method")
                    argcount = func.im_func.func_code.argcount - 1
                else:
                    self.logger.debug("its a function")
                    argcount = func.func_code.argcount
                if argcount == 0:
                """
                args, varargs, _, _ = getargspec(func)

                if not varargs and (not args or (ismethod(func) and len(args) == 1)):
                    return func()
    ...
    def __exit__(self, exc_type, exc_value, exc_tb):
        if self.isPending():
            if exc_value is not None:
                self.reject(exc_value) #TODO: check if this works
                return True
            else:
                self.fulfill(None)
    ...
    def then(self, success=None, failure=None, name=None):
        ...
        def callAndFulfill(v):
            ...
            try:
                ...
                self.logger.exception("Rejecting from callAndFulfill")
                ret.reject(e)
        ...
        def callAndReject(r):
            ...
            try:
                ...
                self.logger.exception("Rejecting from callAndReject")
                ret.reject(e)
        ...
        if self._state==self.PENDING:
            ...
            """
            #self.logger.debug("Already fulfilled")
            try:
                if _isFunction(success):
                    newvalue = self._invoke(success, self.value)
                    if _isPromise(newvalue):
                        newvalue.then(lambda v: ret.fulfill(v),
                                      lambda r: ret.reject(r, bubbling=True))
                    else:
                        ret.fulfill(newvalue)
                else:
                    # From 3.2.6.4
            ...
            #self.logger.debug("Already rejected. Handler: %s", failure)
            """
            ...
            try:
                ...
                #elif failure!=None:
                    # From 3.2.6.5
                    ret.reject(self.reason, bubbling=True)
                #else:
                #    pass
            except Exception, e:
                ret.reject(e)
