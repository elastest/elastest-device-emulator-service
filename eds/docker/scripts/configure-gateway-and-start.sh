...
CONFIG_FILE="/etc/openmtc/gevent/config-gscl.json"
...
# defaults logging
LOGGING_FILE=${LOGGING_FILE-"/var/log/openmtc/gscl.log"}
...
ONEM2M_REMOTE_CSE_OWN_POA="http://${HOST_NAME}:${ONEM2M_HTTP_TRANSPORT_PORT}"
ONEM2M_REMOTE_CSE_BASE=${ONEM2M_REMOTE_CSE_BASE-"onem2m"}
ONEM2M_REMOTE_CSE_TYPE=${ONEM2M_REMOTE_CSE_TYPE-"IN_CSE"}
...
JQ_STRING=${JQ_STRING}' |
    ...
    (.plugins.openmtc_cse[] | select(.name == "RegistrationHandler") | .disabled) |= '${ETSI_REGISTRATION_DISABLED}' |
...
exec python -m openmtc_gevent.gscl_main $@
