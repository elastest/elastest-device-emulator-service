...
# TODO: kca: closely examine what this does and if we should pass any params
# http://www.gevent.org/gevent.monkey.html
gevent.monkey.patch_all()

from openmtc_scl.confighelper import prepare_config
...
def load_plugin(api, category, descriptor, global_config, etsi_config,
                onem2m_config, is_gateway):
    ...
    try:
        ...
        if config.setdefault("etsi", etsi_config) is not etsi_config:
            raise ConfigurationError("Invalid (reserved) key in configuration "
                                     "for plugin %s: 'etsi'", name)
...
def load_plugins(api, plugins, global_config, etsi_config, onem2m_config,
                 is_gateway):
    for category in plugins.values():
        for plugin in category:
            load_plugin(api, category, plugin, global_config, etsi_config,
                        onem2m_config, is_gateway)
...
def load_config(name):
    ...
    from openmtc_scl.configuration import ETSIConfiguration

    MainConfiguration.__options__["etsi"] = SimpleOption(ETSIConfiguration)
    try:
        with open(name) as f:
            config = json.load(f)
...
def main(default_config_file, is_gateway):
    ...
    logger.debug("Running SCL")

    import signal

    from gevent import spawn_later
    from gevent.event import Event as GEventEvent
    from openmtc_server.transportdomain import EndpointEvent, ConnectorEvent

    from openmtc_gevent.TaskRunner import GEventTaskRunner

    from openmtc_server.transportdomain import TransportDomain
    from openmtc_cse.transport import OneM2MTransportDomain
    from openmtc_scl.transportdomain import ETSITransportDomain

    from openmtc_scl.methoddomain import ETSIMethodDomain
    ...
    emd = ETSIMethodDomain(config=config)
    omd = OneM2MMethodDomain(config=config)

    td = TransportDomain(config=config)
    otd = OneM2MTransportDomain(config=config)
    etd = ETSITransportDomain(config=config)
    ...
    class Api(object):
        ...
        class events(object):
            ...
            connector_created = ConnectorEvent()
            connector_deleted = ConnectorEvent()

            etsi_endpoint_created = EndpointEvent()
            etsi_endpoint_deleted = EndpointEvent()

            onem2m_endpoint_created = EndpointEvent()
            onem2m_endpoint_deleted = EndpointEvent()
        ...
        start_etsi_session = db.start_etsi_session
        start_onem2m_session = db.start_onem2m_session
        get_shelve = db.get_shelve

        # handle requests
        handle_request = td.handle_request
        handle_request_indication = emd.handle_request_indication
        handle_onem2m_request = omd.handle_onem2m_request

        # send requests
        send_request = td.send_request
        send_request_indication = etd.send_request_indication
        send_onem2m_request = otd.send_onem2m_request

        register_point_of_access = otd.register_point_of_access

        # connectors and endpoints
        register_client = td.register_client
        register_onem2m_client = otd.register_client
        register_connector = td.register_connector
        register_retargeting_handler = td.register_retargeting_handler
        unregister_retargeting_handler = td.unregister_retargeting_handler
        get_connectors = td.get_connectors
        get_mid_endpoints = etd.get_mid_endpoints
        get_mid_uri = etd.get_mid_uri
        get_onem2m_endpoints = otd.get_endpoints
        add_poa_list = otd.add_poa_list

        # TODO: decide which it should be
        is_local_path = td.is_local_path
        is_local_path_sync = td.is_local_path

        network_manager = nm

        # TODO: kca: get rid of these two.
        # set_store_forward_cb = etd.set_store_forward_cb
        # register_connectivity_handler = etd.register_connectivity_handler
    ...
    openmtc_scl.api.api = Api
    openmtc_scl.api.events = Api.events
    ...
    try:
        init_component(td, Api)
        init_component(etd, Api)
        init_component(otd, Api)
        init_component(emd, Api)
        ...
        load_plugins(Api, config.get("plugins", ()),
                     config["global"], config["etsi"], config["onem2m"],
                     is_gateway)
        init_plugins()
        start_plugins()

        logger.info("SCL is running")
    except:
