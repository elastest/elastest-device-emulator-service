        ...
        if child_resource:
            try:
                def map_child_resource(v):
                    ...
                    return res_cls(v["name"], resourceID=v["value"],
...
class OneM2MDictSerializer(OneM2MSerializer):
    def encode_resource(self, resource, pretty=False, path=None,
                        encoding="utf-8", fields=None, encapsulated=False):
        ...
        if isinstance(resource, Notification):
            ...
            try:
                ...
                if event:
                    e = event.values
                    r = self.encode_resource(event.representation, pretty,
                                             path, encoding, fields, True)
                    e['representation'] = r
                    representation["notificationEvent"] = {
                        get_short_attribute_name(k) or
        ...
        if isinstance(resource, OneM2MResource):
            ...
            representation["childResource"] = map(
                get_child_rep, representation["childResource"])
            ...
            try:
                ...
                for k, v in o.items():
                    if v is None:
                        ...
                        o[k] = self.encode_resource(v, pretty, path, encoding,
                                                    fields)
                    else:
                        try:
                            if len(v) == 0:
    ...
    def _handle_partial_addressing(self, resource, pretty):
        for k, v in resource.iteritems():
            if k == 'latest' and isinstance(v, ContentInstance):
                v = v.resourceID
                resource[k] = v
            if k == 'oldest' and isinstance(v, ContentInstance):
                v = v.resourceID
                resource[k] = v
        if pretty:
    ...
    def decode_resource_values(self, s):
        ...
        try:
            ...
            raise CSESyntaxError("Not a valid resource representation: %s" % (e.message, ))
