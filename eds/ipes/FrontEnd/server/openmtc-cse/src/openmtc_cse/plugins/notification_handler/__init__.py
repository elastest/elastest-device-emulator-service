...
from openmtc_onem2m.model import (Subscription, Notification,
                                  SubscribableResource, NotificationEventC,
...
def get_event_notification_criteria(subscription):
    ...
    # criteria.
    if subscription.eventNotificationCriteria:
    ...
    # notificationEventType and then continue with the step 2.0
        ...
        event_notification_criteria = EventNotificationCriteria()
...
class NotificationHandler(Plugin):
    def __init__(self, api, config, *args, **kw):
        super(NotificationHandler, self).__init__(api, config, *args,
                                                  **kw)
        # self.notification_uris = {}
    ...
    def _init(self):
        ...
        # resource updated
        self.events.resource_updated.register_handler(
            self._handle_subscribable_resource_updated,
            SubscribableResource
        )

        # resource created
        self.events.resource_created.register_handler(
            self._handle_subscribable_resource_created,
            SubscribableResource
        )

        # resource deleted
        self.events.resource_deleted.register_handler(
            self._handle_subscribable_resource_deleted,
            SubscribableResource
        )
    ...
    def _get_sub_list(self, pid, net):
        return [v["sub"] for v in self.subscriptions_info.values()
                if v["pid"] == pid and net in v["enc"].notificationEventType]

    def _delete_subs_from_parent(self, pid):
        self.subscriptions_info = {k: v for k, v
                                   in self.subscriptions_info.iteritems()
                                   if v["pid"] != pid}

    def _handle_subscription_created(self, subscription, req):

        event_notification_criteria = get_event_notification_criteria(
            subscription)
        ...
        self.subscriptions_info[subscription.resourceID] = {
            ...
            "enc": event_notification_criteria,
            "sub": subscription
        }

    def _handle_subscription_updated(self, subscription, req):
        sub_dict = self.subscriptions_info[subscription.resourceID]
        sub_dict["enc"] = get_event_notification_criteria(subscription)
        # TODO(rst): test this
        sub_dict["sub"] = subscription
    ...
    def _handle_subscription_deleted(self, subscription, req):
        ...
        try:
            ...
            self.logger.debug("subscription target %s already deleted or not "
                              "existing." % su)

    def _handle_subscribable_resource_updated(self, resource, req):

        self.logger.debug("_handle_subscribable_resource_updated for %s, the "
                          "request was: %s", resource, req)

        rid = resource.resourceID
        net_update = NotificationEventTypeE.updateOfResource

        sub_list = self._get_sub_list(rid, net_update)

        for sub in sub_list:
            self._handle_subscription(resource, sub, req)

    def _handle_subscribable_resource_created(self, resource, req):
        self.logger.debug("_handle_subscribable_resource_created for %s, the "
                          "request was: %s", resource.get_values(), req)

        pid = resource.parentID
        net_create_child = NotificationEventTypeE.createOfDirectChildResource

        sub_list = self._get_sub_list(pid, net_create_child)

        for sub in sub_list:
            self._handle_subscription(resource, sub, req)

    def _handle_subscribable_resource_deleted(self, resource, req):
        self.logger.debug("_handle_subscribable_resource_deleted for %s, the "
                          "request was: %s", resource, req)
        ...
        sub_list = (self._get_sub_list(rid, net_delete) +
                    self._get_sub_list(pid, net_delete_child))
    ...
    def _handle_subscription(self, resource, sub, req):
        self.logger.debug("_handle_subscription: %s, %s", sub.get_values(), req)
        ...
        if not sub.notificationContentType:
            notification_content_type = NotificationContentTypeE.allAttributes
        else:
            notification_content_type = sub.notificationContentType
        ...
        if notification_content_type is NotificationContentTypeE.allAttributes:
            pass
        ...
        #   attribute(s) only
        elif notification_content_type is NotificationContentTypeE.modifiedAttributes:
            pass
        ...
        elif notification_content_type is NotificationContentTypeE.resourceID:
            pass
        ...
        self._send_notification(resource, sub, req)
