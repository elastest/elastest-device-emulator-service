...
class AccelerometerSensor(XAE):

    def __init__(self, *args, **kw):
        super(AccelerometerSensor, self).__init__(*args, **kw)
    ...
    def _on_register(self):
        ...
        self.request_cnt = Container(resourceName='request',
                labels=['elastest:sensor:accelerometer:request'],
                maxNrOfInstances=5)
        ...
        self.response_cnt = Container(resourceName='response',
                labels=['elastest:sensor:accelerometer:response'],
                maxNrOfInstances=5)
        ...
        self.status_cnt = Container(resourceName='status',
                labels=['elastest:sensor:accelerometer:status'],
                maxNrOfInstances=5)
    ...
    def handle_request(self, cnt, con):
        ...
        status_data = [{'n': 'accelerometer_sensor',
            't': timestamp,
            's': str(self.status)}]
    ...
    def add_sensor_method(self, path, name):
        ...
        sensor_cnt = Container(resourceName=sensor_name,
                labels=['elastest:sensor:accelerometer'],
                maxNrOfInstances=1)
        ...
        data_cnt = Container(resourceName='data',
                labels=['elastest:sensor:accelerometer:data'],
                maxNrOfInstances=5)
        ...
        def add_update_method(func, name):
            ...
            data = [uniform(float(conf['min']), float(conf['max'])), uniform(float(conf['min']), float(conf['max'])),
                    uniform(float(conf['min']), float(conf['max']))]
