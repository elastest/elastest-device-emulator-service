...
class OneM2MMethodDomain(Component):
    ...
    def init_cse_base(self):
        ...
        cse_base_name = onem2m_config.get("cse_base", "onem2m")
        ...
        if path.startswith('C'):
            ...
            if operation == OneM2MOperation.create:
                if isinstance(onem2m_request.content, model.Subscription):
                    ...
                    cn.notificationURI = [pre + uri
                                          for uri in cn.notificationURI]
                    if cn.subscriberURI:
                        cn.subscriberURI = pre + cn.subscriberURI

            # adjust originator
            if onem2m_request.originator:
                onem2m_request.originator = pre + onem2m_request.originator
    ...
    def _handle_onem2m_request(self, db_session, request):
        ...
        resource = self._check_existence(db_session, path)  # todo (hve): (readability) confusing name for function
